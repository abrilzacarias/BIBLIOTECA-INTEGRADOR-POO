        ...
        success: function(data) {
            ...
            if (data.length === 0) {
                ...
                data.forEach(descripcion => {
                    ...
                    checkbox.value = descripcion.id;
                    checkbox.id = descripcion.id;

                    const label = document.createElement('label');
                    label.textContent = `${descripcion.descripcion}`; // Muestra tanto la descripción como el ID

                    // Estilo CSS para alinear elementos horizontalmente y ajustar el espaciado
                    checkbox.style.display = 'inline-block';

                    label.style.display = 'inline-block';
                    label.style.width = '200px'; // Establece un ancho fijo para el label
                    ...
                    contenedor.style.justifyContent = 'flex-start'; // Alinea el contenido a la izquierda
                ...
                agregarVariableBtn.className = 'btn btn-success';
                agregarVariableBtn.style.marginLeft = '400px'; // Ajusta este valor para mover el botón a la derecha
...
$(document).ready(function() {
    $('.opcionesInput').on('input', function() {
        var inputText = $(this).val();
        var capitalizedText = inputText.replace(/\w\S*/g, function(txt) {
...
$('#buscar-libro-input').on('input', function() {
    var inputText = $(this).val();
    var capitalizedText = inputText.replace(/\w\S*/g, function(txt) {
        return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
    });
...
$('#buscar-libro-btn').click(function(event) {
    ...
    $.ajax({
        ...
        contentType: "application/json",
        success: function(data) {
            console.log("Respuesta de buscarPorTitulo:", data);
            // Limpia la tabla actual
            $('#tabla-resultados').empty();
